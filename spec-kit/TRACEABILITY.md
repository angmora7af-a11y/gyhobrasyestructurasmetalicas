...
| FR | Descripción breve | HU | Código / notas | Validación sugerida |
|:---|:---|:---|:---|:---|
| FR-001 | Login, sesión, recovery | HU-002, HU-005 | `apps/web` login + Zod; API auth pendiente | pytest API auth; E2E login |
| FR-002 | RBAC | HU-003 | `apps/web` `navConfig`, `AreaGuard` | pytest permisos por rol; E2E rutas bloqueadas |
| FR-003 | Auditoría | HU-004 | — | pytest insert audit log; revisión tabla |
| FR-004 | Perfiles | HU-003 | `types/role.ts` | matriz roles en test parametrizado |
...
| FR-030–033 | Tickets | HU-012, HU-013 | rutas placeholder `/cliente` | pytest ticket FSM; fechas SLA |
...
| FR-070–073 | Proforma / export Siigo | HU-018 | política PDR (sin CPE en app) | assert no endpoint CPE; export file |
...
| FR-090–092 | Dashboards / BI | HU-022, HU-023 | `AdminDashboardPage` placeholders | API agregados; smoke |
